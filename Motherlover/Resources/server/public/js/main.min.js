...
/*   26 */        // skip empty text layers
/*   27 */        if (layer.kind === 'LayerKind.TEXT' && layer.textContent === '') return
/*   28 */  
/*   29 */        var element = updateLayerElement(elements[layer.id], layer)
/*   30 */        if (!element.parent().length) {
/*   31 */          elements[layer.id] = element.appendTo(canvasElement)
/*   32 */        }
/*   33 */        layersById[layer.id] = layer
/*   34 */      })
/*   35 */    })
/*   36 */  
/*   37 */    socket.on('layerPngReady', function (layerId) {
/*   38 */      elements[layerId].css({
/*   39 */        backgroundColor: 'transparent',
/*   40 */        backgroundImage: 'url("images/layers/' + layersById[layerId].name + '")'
/*   41 */      })
/*   42 */    })
/*   43 */  
/*   44 */    socket.emit('ready')
/*   45 */  
/*   46 */    function purgeLayers(newLayerSet) {
/*   47 */      var ids = newLayerSet.map(function (layer) { return layer.id })
/*   48 */  
/*   49 */      layersArray.forEach(function (layer) {
/*   50 */        // skip empty text layers
/*   51 */        if (layer.kind === 'LayerKind.TEXT' && layer.textContent === '') return
/*   52 */  
/*   53 */        var layerId = layer.id
/*   54 */  
/*   55 */        if (ids.indexOf(layerId) === -1 || isFormatChanged(layer)) {
/*   56 */          elements[layerId].remove()
/*   57 */          ;delete elements[layerId]
/*   58 */        }
/*   59 */      })
/*   60 */    }
/*   61 */  
/*   62 */    function isFormatChanged(layer) {
/*   63 */      var element = elements[layer.id]
/*   64 */      var className = layer.kind.replace('LayerKind.', '').toLowerCase()
/*   65 */      return (element !== undefined && !element.hasClass(className))
/*   66 */    }
/*   67 */  
/*   68 */    function updateLayerElement(element, layer) {
/*   69 */      var styles = {
/*   70 */        left: layer.bounds[0] + 'px',
/*   71 */        top: layer.bounds[1] + 'px',
/*   72 */        width: (layer.bounds[2] - layer.bounds[0])  + 'px',
/*   73 */        height: (layer.bounds[3] - layer.bounds[1])  + 'px',
/*   74 */        display: layer.isVisible ? 'block' : 'none',
/*   75 */        zIndex: layer.index,
/*   76 */        opacity: layer.opacity / 100
/*   77 */      }
/*   78 */  
/*   79 */      switch (layer.kind) {
/*   80 */        case 'LayerKind.TEXT':
/*   81 */          styles.width = (parseInt(styles.width, 10) + 10) + 'px'
/*   82 */          element = element || $('<div id="ml-' + layer.id + '" class="text"></div>')
/*   83 */          element.empty().append(createParagraphs(layer))
/*   84 */          element.css(styles)
/*   85 */          break
/*   86 */        case 'LayerKind.SOLIDFILL':
/*   87 */          styles.backgroundColor = layer.fillColor
/*   88 */          element = element || $('<div id="ml-' + layer.id + '" class="solidfill"></div>')
/*   89 */          createDropShadow(styles, layer)
/*   90 */          element.css(styles)
/*   91 */          break
/*   92 */        case 'LayerKind.NORMAL':
/*   93 */          styles.backgroundColor = 'transparent'
/*   94 */          styles.backgroundImage = 'url("images/layers/' + layer.name + '")'
/*   95 */          element = element || $('<div id="ml-' + layer.id + '" class="normal"></div>')
/*   96 */          element.css(styles)
/*   97 */          break
/*   98 */      }
/*   99 */  
/*  100 */      return element
/*  101 */    }
/*  102 */  
/*  103 */    function createParagraphs(layer) {
/*  104 */      var textStyles = layer.textStyles.slice(0)
/*  105 */      var textContent = layer.textContent
/*  106 */      var paragraphs = layer.paragraphStyles.map(function (paragraphStyle) {
/*  107 */        var spans = createSpansForParagraphs(textStyles, paragraphStyle.from, paragraphStyle.to, textContent, layer.transform)
/*  108 */        var p = $('<p>').css({
/*  109 */          textAlign: paragraphStyle.align,
/*  110 */          webkitHyphens: 'auto'
/*  111 */        }).append(spans)
/*  112 */  
/*  113 */        return p[0]
/*  114 */      })
/*  115 */  
/*  116 */      return paragraphs
/*  117 */    }
/*  118 */  
/*  119 */    function createSpansForParagraphs(textStyles, from, to, textContent, transform) {
/*  120 */      var textStyle, spanText, css, spans = []
/*  121 */      var stylesRanges = []
/*  122 */  
/*  123 */      for (var i = 0, l = textStyles.length; i < l; i++) {
/*  124 */        textStyle = textStyles[i]
/*  125 */  
/*  126 */        // hack related to https://github.com/kilianc/motherlover/issues/33
/*  127 */        if (stylesRanges.indexOf(textStyle.from + ':' + textStyle.to) !== -1) continue
/*  128 */        stylesRanges.push(textStyle.from + ':' + textStyle.to)
/*  129 */  
/*  130 */        if (textStyle.to < from) continue
/*  131 */        if (textStyle.from > to) break
/*  132 */  
/*  133 */        spanText = textContent.substring(Math.max(textStyle.from, from), Math.min(textStyle.to, to)).replace('\n', '<br>')
/*  134 */  
/*  135 */        if (transform && transform.xx.toFixed(2) === transform.yy.toFixed(2)) {
/*  136 */          textStyle.size = Math.abs(Math.round(textStyle.size * transform.xx))
/*  137 */        }
/*  138 */  
/*  139 */        css = {
/*  140 */          font: textStyle.size + 'px "' + textStyle.fontPostScriptName + '"',
/*  141 */          color: textStyle.color
/*  142 */        }
/*  143 */  
/*  144 */        css.lineHeight = textStyle.leading + 'px'
/*  145 */        css.letterSpacing = (6 * textStyle.tracking / 200) + 'px'
/*  146 */  
/*  147 */        if (textStyle.underline === 'underlineOnLeftInVertical') {
/*  148 */          css.textDecoration = 'underline'
/*  149 */        }
/*  150 */  
/*  151 */        if (textStyle.strikethrough === 'xHeightStrikethroughOn') {
/*  152 */          css.textDecoration = css.textDecoration ? css.textDecoration + ' line-through' : 'line-through'
/*  153 */        }
/*  154 */  
/*  155 */        if (textStyle.fontCaps === 'allCaps') {
/*  156 */          css.textTransform = 'uppercase'
/*  157 */        } else if (textStyle.fontCaps === 'smallCaps') {
/*  158 */          css.fontVariant = 'small-caps'
/*  159 */        }
/*  160 */  
/*  161 */        spans.push($('<span>').text(spanText).css(css)[0])
/*  162 */      }
/*  163 */  
/*  164 */      return spans
/*  165 */    }
/*  166 */  
/*  167 */    function createDropShadow(styles, layer) {
/*  168 */      if (!layer.layerStyles || !layer.layerStyles.dropShadow) {
/*  169 */        styles.boxShadow = 'none'
/*  170 */        return
/*  171 */      }
/*  172 */  
/*  173 */      var dropShadowData = layer.layerStyles.dropShadow
/*  174 */      var angle = (180 - (dropShadowData.useGlobalAngle ? dropShadowData.globalAngle : dropShadowData.localLightingAngle)) * Math.PI / 180
/*  175 */      var x = Math.round(Math.cos(angle) * dropShadowData.distance)
/*  176 */      var y = Math.round(Math.sin(angle) * dropShadowData.distance)
/*  177 */      var chokeMatte = dropShadowData.size * dropShadowData.chokeMatte / 100
/*  178 */      var blur = dropShadowData.size - chokeMatte
/*  179 */      var rgb = dropShadowData.color
/*  180 */      var rgba = 'rgba(' + parseInt(rgb.slice(1, 3), 16) + ', ' + parseInt(rgb.slice(3, 5), 16) + ', ' + parseInt(rgb.slice(5, 7), 16) + ', ' + dropShadowData.opacity / 100 + ')'
/*  181 */  
/*  182 */      styles.boxShadow = x + 'px ' + y + 'px ' + blur + 'px ' + chokeMatte + 'px ' + rgba
/*  183 */      alert(styles.boxShadow)
/*  184 */    }
/*  185 */  })
