...
var TRY_CATCH_TPL = '[JS_CODE]' // neste try catch just doesn't work =|
...
Photoshop.prototype.dispatchMessage = function dispatchMessage() {
  var self = this
  var body = self.incomingMessage.body

  if (Photoshop.NO_COMM_ERROR !== self.incomingMessage.commStatus || /^Error:/.test(body)) {
    var err = new Error()
    err.name = 'PhotoshopError'
    err.message = body.slice(6)
  }
  ...
  if (subscriptionType && ~self.pubSubEvents.indexOf(subscriptionType)) {
    self.dispatchEvents(subscriptionType, parts)
  } else if (/^{"sendDocumentInfoToNetworkClient/.test(body)) {
    self.emit('documentInfo', JSON.parse(body))
  } else {
...
Photoshop.prototype.dispatchEvents = function dispatchEvents(subscriptionType, parts) {
  if (0 === parts.length) return

  var self = this

  if ('imageChanged' === subscriptionType) {
    var imageChangedData = JSON.parse(parts[0])
    var documentId = imageChangedData.documentID
    var events = imageChangedData.layerEvents || []

    // normalize data structure
    imageChangedData.imageEvents && events.push(imageChangedData.imageEvents)

    events.forEach(self.dispatchEvent.bind(self, documentId))
  }
...
Photoshop.prototype.dispatchEvent = function dispatchEvent(documentId, eventGroup) {
  var self = this
  var layerId = eventGroup.layerID

  Object.keys(eventGroup).forEach(function (eventName) {
    if ('layerID' === eventName) return

    var eventData = { documentId: documentId }
    var data = eventGroup[eventName]
...
Photoshop.prototype.destroy = Photoshop.prototype.close = function () {
